...
/**
 * Shell ls command - list directory contents
 */
void vfs_shell_ls(const char* path) {
    char** entries;
    uint32_t count;
    
    int result = vfs_list_directory(path, &entries, &count);
    
    if (result != VFS_SUCCESS) {
        switch (result) {
            case VFS_ERROR_NOT_FOUND:
                zprint("ls: directory not found\n");
                break;
            case VFS_ERROR_NOT_DIR:
                zprint("ls: not a directory\n");
                break;
            default:
                zprint("ls: error listing directory\n");
                break;
        }
        return;
    }
...
/**
 * Shell mkdir command - create directory
 */
void vfs_shell_mkdir(const char* path) {
    ...
    int result = vfs_create_directory(path, PERM_READ | PERM_WRITE | PERM_EXECUTE);
    
    if (result != VFS_SUCCESS) {
        switch (result) {
            case VFS_ERROR_EXISTS:
                zprint("mkdir: directory already exists\n");
                break;
            case VFS_ERROR_NOT_FOUND:
                zprint("mkdir: parent directory not found\n");
                break;
            case VFS_ERROR_NO_MEM:
                zprint("mkdir: out of memory\n");
                break;
            default:
                zprint("mkdir: error creating directory\n");
                break;
        }
    } else {
        zprint("Directory created successfully\n");
    }
}

/**
 * Shell touch command - create file
 */
void vfs_shell_touch(const char* path) {
    ...
    if (result != VFS_SUCCESS) {
        switch (result) {
            case VFS_ERROR_EXISTS:
                zprint("touch: file already exists\n");
                break;
            case VFS_ERROR_NOT_FOUND:
                zprint("touch: parent directory not found\n");
                break;
            case VFS_ERROR_NO_MEM:
                zprint("touch: out of memory\n");
                break;
            default:
                zprint("touch: error creating file\n");
                break;
        }
    } else {
        zprint("File created successfully\n");
    }
}

/**
 * Shell rm command - remove file
 */
void vfs_shell_rm(const char* path) {
    ...
    vfs_node_t* node = vfs_find_node(path);
    if (!node) {
        zprint("rm: file not found\n");
        return;
    }
    
    int result;
    if (node->type == FILE_TYPE_DIRECTORY) {
        result = vfs_delete_directory(path);
    } else {
        result = vfs_delete_file(path);
    }
    ...
    if (result != VFS_SUCCESS) {
        switch (result) {
            case VFS_ERROR_NOT_FOUND:
                zprint("rm: file not found\n");
                break;
            case VFS_ERROR_NO_PERM:
                zprint("rm: directory not empty\n");
                break;
            default:
                zprint("rm: error removing file\n");
                break;
        }
    } else {
        zprint("File removed successfully\n");
    }
}

/**
 * Shell cd command - change directory
 */
void vfs_shell_cd(const char* path) {
    ...
    int result = vfs_change_directory(path);
    
    if (result != VFS_SUCCESS) {
        switch (result) {
            case VFS_ERROR_NOT_FOUND:
                zprint("cd: directory not found\n");
                break;
            case VFS_ERROR_NOT_DIR:
                zprint("cd: not a directory\n");
                break;
            default:
                zprint("cd: error changing directory\n");
                break;
        }
    }
}
...
/**
 * Shell pwd command - print working directory
 */
void vfs_shell_pwd(void) {
    char* current_dir = vfs_get_current_directory();
    if (current_dir) {
        zprint(current_dir);
        zprint("\n");
    } else {
...
/**
 * Shell cat command - display file contents
 */
void vfs_shell_cat(const char* path) {
    ...
    if (fd < 0) {
        switch (fd) {
            case VFS_ERROR_NOT_FOUND:
                zprint("cat: file not found\n");
                break;
            case VFS_ERROR_IS_DIR:
                zprint("cat: is a directory\n");
                break;
            default:
                zprint("cat: error opening file\n");
                break;
        }
        return;
    }
...
/**
 * Shell echo command - write text to file
 */
void vfs_shell_echo(const char* text, const char* path) {
    if (!text || !path) {
        zprint("echo: missing text or file name\n");
        return;
    }
    ...
    if (fd < 0) {
        switch (fd) {
            case VFS_ERROR_IS_DIR:
                zprint("echo: is a directory\n");
                break;
            case VFS_ERROR_NO_MEM:
                zprint("echo: out of memory\n");
                break;
            default:
                zprint("echo: error opening file\n");
                break;
        }
        return;
    }
    ...
    int bytes_written = vfs_write(fd, text, text_len);
    
    if (bytes_written != text_len) {
        zprint("echo: error writing to file\n");
    }
