...
void print_module_info(struct multiboot_info * info) {
	...
	if(info->mods_count > 0 ){
		...
		for(int i = 0; i < (info->mods_count) ; i++){
			...
			print_log("Mod Start (Virtual) : 0x"); print_log_int(module->mod_start,16);  print_log("\n");
			print_log("Mod End (Virtual) : 0x"); print_log_int(module->mod_end,16);  print_log("\n");
...
void print_multiboot_info(struct multiboot_info *mboot_ptr){
	...
	print_log("cmdline : 0x");			print_log_int(mboot_ptr->cmdline,16);			print_log("\n");
	print_log("mods_count : ");			print_log_int(mboot_ptr->mods_count,10);		print_log("\n");
	print_log("mods_addr : 0x");		print_log_int(mboot_ptr->mods_addr,16);			print_log("\n");
